...
struct DiagnosticNetworkView: View {
    ...
    @State private var copyBusy = false
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                .controlSize(.small)
                .disabled(copyBusy || clearBusy)
                .help("采集网络/故障切换状态与最近日志，复制到剪贴板便于发给开发者分析")
                Button {
                    loadDiagnostic()
                } label: {
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    copyMessage = "已复制诊断日志（\(text.count) 字符），可直接粘贴发给开发者"
                    copyBusy = false
                }
            } catch {
                await MainActor.run {
                    copyMessage = "复制失败：\(error.localizedDescription)"
                    copyBusy = false
                }
            }
        }
    ...
    private func clearDiagnosticsLog() {
        guard case .running = backend.state else {
            copyMessage = "后台未运行，无法清空"
            return
        }
        clearBusy = true
        Task {
            do {
                ...
                await MainActor.run {
                    copyMessage = "清空失败：\(error.localizedDescription)"
                    clearBusy = false
                }
            }
        }
