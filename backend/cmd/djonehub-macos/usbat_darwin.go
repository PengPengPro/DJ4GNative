...
func openDJIUSBAT() (*usbAT, error) {
	...
	for _, ids := range usbDeviceIDs {
		...
		if handle == nil {
			if lastErr == nil {
				lastErr = fmt.Errorf("DJI USB AT device %04x:%04x not found", ids[0], ids[1])
			}
			continue
		}
	...
	return nil, errors.New("no USB AT device found")
}

// probeUSBATHandle 在已打开的 USB 设备上探测 AT bulk 接口
func probeUSBATHandle(ctx *C.libusb_context, handle *C.libusb_device_handle) (*usbAT, error) {
	...
	return nil, errors.New("no USB bulk interface candidates found for AT bridge")
}
...
func (u *usbAT) bulkWriteLocked(endpoint byte, payload []byte, timeout time.Duration) error {
	err := u.bulkTransferWriteLocked(endpoint, payload, timeout)
	if err == nil || !isUSBEndpointStalled(err) {
		return err
	}
	u.clearHaltLocked(endpoint)
	return u.bulkTransferWriteLocked(endpoint, payload, timeout)
}

func (u *usbAT) bulkReadLocked(endpoint byte, timeout time.Duration) ([]byte, error) {
	data, err := u.bulkTransferReadLocked(endpoint, timeout)
	if err == nil || errors.Is(err, errUSBTimeout) || !isUSBEndpointStalled(err) {
		return data, err
	}
	u.clearHaltLocked(endpoint)
	return u.bulkTransferReadLocked(endpoint, timeout)
}

func (u *usbAT) bulkTransferWriteLocked(endpoint byte, payload []byte, timeout time.Duration) error {
...
func (u *usbAT) bulkTransferReadLocked(endpoint byte, timeout time.Duration) ([]byte, error) {
	buf := make([]byte, 512)
	var transferred C.int
	rc := C.libusb_bulk_transfer(
		u.handle,
...
func (u *usbAT) clearHaltLocked(endpoint byte) {
	if u.handle == nil {
		return
	}
	if rc := C.libusb_clear_halt(u.handle, C.uchar(endpoint)); rc != 0 {
		logUSBATRecovery("clear halt on endpoint 0x%02x failed: %s", endpoint, usbErrorName(rc))
	}
}

func isUSBEndpointStalled(err error) bool {
	if err == nil {
		return false
	}
...
func logUSBATRecovery(format string, args ...any) {
	log.Printf("USB AT recovery: "+format, args...)
}
